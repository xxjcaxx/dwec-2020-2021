...
class viewPlanet {
    ...
    viewDetails() {
        ...
        data.datasets.push({label:'Emission',data:emission, borderColor: 'rgba(255,99,132,1)', fill:false,borderWidth:2})

        //greenhouse
        let greenhouse = this.planet.planetaryChanges.map((p)=>p.greenhouse)
        data.datasets.push({label:'Greenhouse',data:greenhouse, borderColor: 'rgba(255,255,99,1)', fill:false,borderWidth:2})

         //temperature
         let temp = this.planet.planetaryChanges.map((p)=>p.average_temperature)
         data.datasets.push({label:'Temperature',data:temp, borderColor: 'rgba(99,255,99,1)', fill:false,borderWidth:2})

          //co2
        let co2 = this.planet.planetaryChanges.map((p)=>p.co2)
        data.datasets.push({label:'Co2',data:co2, borderColor: 'rgba(99,99,99,1)', fill:false,borderWidth:2})

         //oxigen
         let oxigen = this.planet.planetaryChanges.map((p)=>p.oxigen)
         data.datasets.push({label:'Oxigen',data:greenhouse, borderColor: 'rgba(99,99,255,1)', fill:false,borderWidth:2})

          //plants
        let plants = this.planet.planetaryChanges.map((p)=>p.plants)
        data.datasets.push({label:'Plants',data:plants, borderColor: 'rgba(150,255,160,1)', fill:false,borderWidth:2})

                  //animals
        let animals = this.planet.planetaryChanges.map((p)=>p.animals)
        data.datasets.push({label:'Animals',data:animals, borderColor: 'rgba(150,150,0,1)', fill:false,borderWidth:2})
